...
export async function getSessionUser(db: D1Database, sessionId: string | undefined): Promise<any> {
	...
	const result = await db
		.prepare(
			'SELECT users.id, users.email, users.name, users.avatar_url FROM users JOIN sessions ON users.id = sessions.user_id WHERE sessions.id = ? AND datetime(sessions.expires_at) > datetime("now")',
		)
		.bind(sessionId)
		.first();
